...
def parse_points(df):
    ...
    """
    part_one = df.select(sql_functions.split(df.value, ',').alias('value'))
    part_two = part_one.map(lambda row: LabeledPoint(float(row['value'][0]), (row['value'][1:])))
    return part_two.toDF(['features', 'label'])

parsed_points_df = parse_points(raw_data_df)
first_point_features = parsed_points_df.select(parsed_points_df['features']).collect()[0]
first_point_label = parsed_points_df.select(parsed_points_df['label']).collect()[0][0]
print first_point_features, first_point_label

d = len(first_point_features.features)
print d
...
min_year = content_stats.select('min(label)').collect()[0][0]
max_year = content_stats.select('max(label)').collect()[0][0]
...
parsed_data_df = (parsed_points_df
                  .select(parsed_points_df.features, (parsed_points_df.label-min_year).alias('label'))
                  )
...
# TODO: Replace <FILL IN> with appropriate code
preds_and_labels_train = parsed_train_data_df.map(lambda obj: (obj.label, average_train_year))
...
preds_and_labels_val = parsed_val_data_df.map(lambda obj: (obj.label, average_train_year))
...
preds_and_labels_test = parsed_test_data_df.map(lambda obj: (obj.label, average_train_year))
...
def get_labeled_prediction(weights, observation):
    ...
    """
    a = weights.dot(observation.features)
    b = observation.label
