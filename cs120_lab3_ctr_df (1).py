# Databricks notebook source exported at Thu, 27 Oct 2016 04:47:36 UTC
# MAGIC %md
...
def one_hot_encoding(raw_feats, ohe_dict_broadcast, num_ohe_feats):
    ...
    for x in raw_feats:
      if x in ohe_dict_broadcast.value:
        m_list.append((ohe_dict_broadcast.value[x], 1.0))
    return SparseVector(num_ohe_feats, m_list)
...
sample_distinct_feats_df = (sample_data_df
                            .select(explode(sample_data_df.features).alias('features'))
                            .distinct()
                            )
...
def parse_raw_df(raw_df):
    ...
    """
    return raw_df.select( split(raw_df.text, ',').getItem(0).alias('label'), parse_point_udf(raw_df.text).alias('feature') )
...
ohe_train_df = (parsed_train_df
                  .select('label', ohe_dict_udf(parsed_train_df.feature).alias('features'))
                ).cache()
